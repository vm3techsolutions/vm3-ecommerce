...
const WebsiteDevelopment = () => {
  ...
  const wishlistItems = useSelector((state) => state.wishlist.items); // contains full packages
    ...
    const fetchPackages = async () => {
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/packages/category/1`
        );
                ...
                {pkg.details.recommended_addons.map((addon, index) => (
                  <label key={index} className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      checked={
                        selectedAddons[pkg.id]?.includes(addon.name) || false
                      }
