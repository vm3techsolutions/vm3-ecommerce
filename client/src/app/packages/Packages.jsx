  ...
  return (
    ...
    <section
      ...
      {categories.map((category) => (
        <div key={category.id} className="bg-black text-white p-6 rounded-xl shadow-lg">
          <h3 className="text-xl font-bold mb-2 pb-2">{category.name}</h3>
          <p className="mb-4 pb-10">{category.description}</p>
         
<PrimaryButton href={`/packages/${category.id}`} className="pb-2 ">
  View Plans
</PrimaryButton>
...
{/* icon Boxes */}
<div>
<h3 className=" text-[#FEC63F] font-bold text-2xl sm:text-4xl pt-25 pb-12 text-center">Why Choose PAYG</h3>
